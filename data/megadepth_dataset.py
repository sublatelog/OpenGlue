...
#         K0_inv_t = torch.transpose(K0_inv, 1, 2).contiguous()
        ...
        """
        R1
        [[ 0.9931465  -0.064502   -0.09746581]
         [ 0.05923886  0.9966739  -0.05596409]
         [ 0.10075142  0.04980678  0.99366415]]
        (3, 3)
        
        T1
        [1.659208  0.8268426 2.7848856]
        (3,)
        """
        R = np.dot(R1, R0.T)
...
#         T = torch.matmul(T1, np.linalg.inv(T0)).detach().numpy().copy()
#         T = (T1 - np.dot(R, T0)).flatten()
#         T = (T1 - np.dot(R, T0)).reshape(3, )
        T = (np.dot(R, T1) - np.dot(R, T0)).reshape(3, )
#         T = (T1 - T0).reshape(3, )
        print("T")
        print(T)
